...
func setupPluginRouter(jm *jwt.Manager) *gin.Engine {
	...
	{
		v1.POST("/route", middleware.RequireAuth(jm, nil), RegisterPlugin)
		v1.GET("/routes", ListPlugins)
		v1.PUT("/route/:slug", middleware.RequireAuth(jm, nil), UpdatePlugin)
		v1.DELETE("/route/:slug", middleware.RequireAuth(jm, nil), DeletePlugin)
	}
	return router
}
...
func TestRegisterPlugin_SuccessAndPersistence(t *testing.T) {
	// Reset global registry (without database persistence for tests)
	plugins.Global = plugins.NewRegistry()
	// Tests use in-memory registry only
...
func TestRegisterPlugin_DuplicateSlug(t *testing.T) {
	// Reset global registry (in-memory for tests)
	plugins.Global = plugins.NewRegistry()
